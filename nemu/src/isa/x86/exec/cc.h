...
static inline void rtl_setcc(DecodeExecState *s, rtlreg_t *dest, uint32_t subcode)
{
  ...
  {
  case CC_O:
    rtl_li(s, dest, eflag(OF) == 1 ? 1 : 0);
    break;
  case CC_B:
    rtl_li(s, dest, eflag(CF) == 1 ? 1 : 0);
    break;
  case CC_E:
    rtl_li(s, dest, eflag(ZF) == 1 ? 1 : 0);
    break;
  case CC_BE:
    rtl_li(s, dest, (eflag(CF) == 1 || eflag(ZF) == 1) ? 1 : 0);
    break;
  case CC_S:
    rtl_li(s, dest, eflag(SF) == 1 ? 1 : 0);
    break;
  case CC_L:
    rtl_li(s, dest, eflag(SF) != eflag(OF) ? 1 : 0);
    break;
  case CC_LE:
    rtl_li(s, dest, (eflag(SF) != eflag(OF) || eflag(ZF) == 1) == 1 ? 1 : 0);
    break;
    // TODO();
  case CC_P:
