...
word_t vaddr_mmu_read(vaddr_t addr, int len, int type) { return 1; };
...
#define def_vaddr_template(bytes)                            \
  ...
  {                                                          \
    ...
    if (ret == MEM_RET_OK)                                   \
      ...
      return vaddr_mmu_read(addr, bytes, MEM_TYPE_READ);     \
    return 0;                                                \
  }                                                          \
  ...
  {                                                          \
    ...
    if (ret == MEM_RET_OK)                                   \
      ...
      return vaddr_mmu_write(addr, bytes, MEM_TYPE_READ);    \
  }
